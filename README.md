...
The CIME submodule under `cime/tools/mapping/gen_domain_files` generates the domain files for CLM. It remains unchanged. You have to compile it in the src/ directory with one of the environments suitable for CLM. Then execute with $MAPFILE being of the mapping files created before and $GRIDNAME being a sting with the name of your grid.
